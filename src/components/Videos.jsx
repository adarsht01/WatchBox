...
const Video = () => {
  ...
  return (
    <Stack direction={['column','row']} h={'100vh'}>
      <VStack w={'full'}>
      <video src={videoSrc} controls autoPlay controlsList='nodownload' style={{width:'100%'}}>

      </video>
      <VStack alignItems={'flex-start'} p={'8'} w={'full'} overflowY={'auto'}>
        <Heading>
          Sample Video {sampleIndex+1}
        </Heading>
        <Text>
        This is a sample video for demo.Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.
        </Text>
      </VStack>
      </VStack>
      <VStack w={['full','xl']} alignItems={'stretch'} p={'12'} spacing={'8'} overflowY={'auto'}>
      {
      videosArr.map((x,index) => (
        <Button onClick={()=>{setVideoSrc(x);setSampleIndex(index)}} variant={'ghost'} colorScheme='purple'>Lecture {index+1}</Button>
      ))
      }
      </VStack>
    </Stack>
